...
class Reservation:
    ...
    def get_date_cancel(self):
        while True:
            date_input = simpledialog.askstring("ezpark", "Fill in your booked date (YYYY-MM-DD):")
            if date_input is None:
                sys.exit()  # Exit if user clicks "Cancel"
            if self.is_valid_date(date_input):
                self.date = date_input
    ...
    def get_time_cancel(self):
        while True:
            time_input = simpledialog.askstring("ezpark", "Enter your booked time (starting from 10:00 - 21:00):")
            if time_input is None:
                sys.exit()  # Exit if user clicks "Cancel"
            if self.is_valid_time(time_input):
                self.time = time_input
    ...
    def store_informations(self):
        with open(reservations_file, "a") as file:
            ...
            file.write(f"{self.username} {self.date} {times_str} {self.slot} {self.duration*self.price} booked\n")

    def cancel_reservation(self):
        self.get_date_cancel()
        self.get_time_cancel()

        with open(reservations_file, "r") as reservation_file:
        ...
        for line in lines:
            ...
            if self.username == name and self.date == date and self.time == time and status != "cancelled":
                # Display information about the booking
                booking_info = f"Username: {name}\nDate: {date}\nStarting time: {time}\nDuration: {duration}\nSlot: {slot}\nTotal payment: {total_payment} baht"
                confirmation = messagebox.askyesno("ezpark", f"Do you want to cancel this booking?\n\n{booking_info}")

                if confirmation:
                    line = line.replace(status, "cancelled")
                    updated_lines.append(line)
                    messagebox.showinfo("ezpark", "Reservation cancelled")
                else:
                    updated_lines.append(line)
            else:
                updated_lines.append(line)
...
todo = simpledialog.askstring("ezpark", "Do you want to make or cencel a reservation? (book or cancel)")
while True:
    if todo.upper().strip() == "CANCEL":
        reservation = Reservation(username)
        reservation.cancel_reservation()
        messagebox.showinfo("ezpark", "All reservations are shown. We hope to see you again")
        break
    elif todo.upper().strip() == "BOOK":
        total_price = 0
        ...
        messagebox.showinfo("ezpark", "Only book or reservation is required")
        todo = simpledialog.askstring("ezpark", "Do you want to make or cencel a reservation? (book or cancel)")
        continue
